...
function MovingBookingContent() {
  ...
  useEffect(() => {
    async function loadService() {
      if (!serviceId) {
        setError("No moving service was selected.");
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (!selectedService) {
          setError(
            "The selected moving service was not found."
          );
          setLoading(false);
          return;
        }
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to load the moving service."
        );
      } finally {
        setLoading(false);
      }
  ...
  /*
   * Preserve the selected moving service when the
   * customer temporarily leaves this page to add
   * an address.
   */
  const returnTo = `/book/moving?service=${encodeURIComponent(
    service.id
  )}`;
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mb-6">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500 sm:text-base">
              Tell us about your move and provide a
              convenient date and location for our team.
            </p>
          </div>
